const concepts = [
  ...
  },
  {
    ...
    body: "Syntax means the rules for writing a program correctly. In C, brackets, semicolons, keywords, and spelling must be written in the correct way.",
    bullets: ["Every statement usually ends with ;", "Code blocks use { }", "Wrong syntax gives a compiler error"]
  },
  {
    title: "What are comments?",
    body: "Comments are notes written inside code for humans to understand. The compiler ignores comments, so they do not change the output.",
    bullets: ["// is a single-line comment", "/* ... */ is a multi-line comment", "Use comments to explain important logic"]
  },
  {
  ...
  {
    title: "What are jumping statements?",
    body: "Jumping statements move the control of the program from one place to another. In C, break, continue, return, and goto are jumping statements.",
    bullets: ["break exits a loop or switch", "continue skips to the next loop round", "return exits from a function"]
  },
  {
  ...
  {
    title: "How to improve programming logic",
    body: "Logic improves when you trace code step by step instead of only reading solutions. Use a table with i, a[i], condition, and output.",
    bullets: ["Dry run with 3 to 5 values", "Write what changes after every loop", "Solve first in normal English, then convert to C"]
  }
];
...
int main() {
  ...
  return 0;
    ...
    answer: "3 6 9 12",
    logic: "i starts at 0 and increases until 3, so the array prints from first to last."
  },
  {
    id: "o2",
    title: "Reverse loop",
...
int main() {
  ...
  return 0;
    ...
    answer: "10 8 6 4 2",
    logic: "The loop starts from the last index, 4, and moves backward to 0."
  },
  {
    id: "o3",
    title: "continue",
...
int main() {
  ...
  return 0;
    ...
    answer: "1 2 4 5",
    logic: "When i becomes 3, continue skips printf and moves to the next iteration."
  },
  {
    id: "o4",
    title: "break",
...
int main() {
  ...
  return 0;
    ...
    answer: "1 2 3",
    logic: "When i becomes 4, break stops the loop completely."
  },
  {
    id: "o5",
    title: "Array update",
...
int main() {
  ...
  return 0;
    ...
    answer: "20",
    logic: "a[1] changes from 10 to a[0] + a[2], which is 5 + 15."
  },
  {
    id: "o6",
...
int main() {
  ...
  return 0;
    ...
    answer: "6",
    logic: "Outer loop runs 3 times. Inner loop runs 2 times for each outer loop. 3 * 2 = 6."
  },
  {
    id: "o7",
    title: "if with array",
...
int main() {
  ...
  return 0;
    ...
    answer: "4 8 10",
    logic: "Only even values pass the condition a[i] % 2 == 0."
  },
  {
    id: "o8",
    title: "Largest trace",
...
int main() {
  ...
  return 0;
    ...
    answer: "18",
    logic: "largest starts as 12. It becomes 18 when index 2 is checked."
  }
];
...
const state = {
  progress: JSON.parse(localStorage.getItem("cSprintProgress") || '{"solved":[],"quizBest":0,"checks":[],"outputs":[]}' ),
  loopIndex: 0,
  playing: null
};

if (!state.progress.outputs) state.progress.outputs = [];
...
function renderOutputTasks() {
  document.querySelector("#outputGrid").innerHTML = outputTasks.map((task) => {
    const done = state.progress.outputs.includes(task.id);
    return `
      <article class="output-card ${done ? "done" : ""}">
        <div class="output-card-head">
          <h3>${task.title}</h3>
          <span class="tag">${done ? "practiced" : "trace first"}</span>
        </div>
        <pre>${escapeHtml(task.code)}</pre>
        <div class="answer-box" id="answer-${task.id}">
          <strong>Output:</strong>
          <code>${escapeHtml(task.answer)}</code>
          <p>${task.logic}</p>
        </div>
        <div class="card-actions">
          <button class="button ghost" data-output="${task.id}">Show answer</button>
          <button class="button primary" data-output-done="${task.id}">${done ? "Done" : "Mark practiced"}</button>
        </div>
      </article>
    `;
  }).join("");
...
document.addEventListener("click", (event) => {
  ...
  const outputDoneId = event.target.dataset.outputDone;
  ...
  if (outputId) {
    document.querySelector(`#answer-${outputId}`).classList.toggle("open");
  }

  if (outputDoneId && !state.progress.outputs.includes(outputDoneId)) {
    state.progress.outputs.push(outputDoneId);
    save();
    renderOutputTasks();
  }
});
